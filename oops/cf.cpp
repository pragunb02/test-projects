...
void solve()
{
    ...
    vector<ll>dp(n+1,maxint);
    dp[1]=0;
    // min-cost path [shortest path]
    ...
    while(pq.size()){
        ...
        pq.pop();
        cout<<"node"<<node<<" "<<dist<<" "<<dp[node]<<endl;
        if(dp[node]!=dist){
            continue;
        }
        if(node>=2 and dp[node-1]>dp[node]){
            dp[node-1]=dp[node]+0;
            pq.push({-dp[node-1],node-1});
        }
        if(dp[b[node]]>dp[node]+a[node]){
            dp[b[node]]=dp[node]+a[node];
            pq.push({-dp[b[node]],b[node]});
        }
    }
    ll ans=0;
    for(int i=1;i<=n;i++){
        ans=max(ans,psum[i]-dp[i]);
    }
    cout<<ans<<endl;
}
